...
gulp.task('swigLinux', run('make -j4 JAVA_HOME=/usr/lib/jvm/java-8-openjdk-amd64 "JAVA_INCLUDE=-I$(JAVA_HOME)/include -I$(JAVA_HOME)/include/linux"', {
  cwd: 'gdal-src/swig/java'
}));
...
gulp.task('copyWindows', ()=> {
  fs.copySync('gdal-win/bin/gdal/java/gdalalljni.dll', 'target/')
});